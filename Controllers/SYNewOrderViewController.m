...
#import "SYNewOrderViewController.h"

@interface SYNewOrderViewController ()<UITableViewDelegate,UITableViewDataSource,MLAMOrderDeliveryTableViewCellDelegate>
...
@implementation SYNewOrderViewController

+ (instancetype)initNewOrderVCFromNibFile {
    SYNewOrderViewController *vc = [[UIStoryboard storyboardWithName:@"Main" bundle:[NSBundle mainBundle]] instantiateViewControllerWithIdentifier:@"SYNewOrderViewController"];
    return vc;
}
...
- (void)configTableView {
    ...
    self.tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        page = 0;
        [kSharedWebService fetchOrdersWithState:SYOrderStateNew page:page completion:^(NSMutableArray *orders, BOOL didReachEnd, NSError *error) {
    ...
    self.tableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        [kSharedWebService fetchOrdersWithState:SYOrderStateNew page:page+1 completion:^(NSMutableArray *orders, BOOL didReachEnd, NSError *error) {
