//
//  MLAMOrderDetailViewController.m
//  MLDelivery
//
...
#import "MLAMOrderDetailViewController.h"
...
@interface MLAMOrderDetailViewController ()<UITableViewDelegate, UITableViewDataSource,UIAlertViewDelegate>
...
@implementation MLAMOrderDetailViewController
+ (instancetype)instantiateFromStoryboard {
    UIStoryboard *sb = [UIStoryboard storyboardWithName:@"Storyboard" bundle:[NSBundle mainBundle]];
    MLAMOrderDetailViewController *VC = [sb instantiateViewControllerWithIdentifier:@"MLAMOrderDetailViewController"];
    return VC;
}
...
- (void)configureTabelView {
    ...
    self.tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        [kSharedWebService fetchOrderDetail:weakSelf.order completion:^(MLAMOrder *order, NSError *error) {
...
- (BOOL)isOrderUncompleted {
    return  (self.order.orderState.integerValue == MLAMOrderStateNew ||
             self.order.orderState.integerValue == MLAMOrderStateGotoTake ||
             self.order.orderState.integerValue == MLAMOrderStateInDelivery);
}

- (void)configureBottomView {
    if (self.order.orderState.integerValue == MLAMOrderStateCompleted) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MLAMMapRouteViewController *controller = [MLAMMapRouteViewController instantiateFromStoryboard];
