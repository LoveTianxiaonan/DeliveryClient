...
- (void)addTopBorderWithColor:(UIColor *)color width:(CGFloat)borderWidth leftMargin:(CGFloat)leftMargin rightMargin:(CGFloat)rightMargin topMargin:(CGFloat)topMargin{
    [self removeTopBorder];
    UIView *border = [UIView autoLayoutView];
    border.userInteractionEnabled = NO;
    border.backgroundColor = color;
    ...
    [self addSubview:border];
    [border pinToSuperviewEdges:JRTViewPinLeftEdge inset:leftMargin];
    [border pinToSuperviewEdges:JRTViewPinTopEdge inset:topMargin];
    [border pinToSuperviewEdges:JRTViewPinRightEdge inset:rightMargin];
    [border constrainToHeight:borderWidth];
}
