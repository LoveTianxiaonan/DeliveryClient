...
#import "MLAMWebService.h"

@implementation MLAMWebService

+ (MLAMWebService *)sharedInstance {
    static MLAMWebService *sharedInstance = nil;
    DEFINE_SHARED_INSTANCE_USING_BLOCK(^{
        sharedInstance = [MLAMWebService new];
...
- (void)fetchHistoryOrdersWithYear:(NSInteger)year month:(NSInteger)month page:(NSInteger)page completion:(void(^)(NSMutableArray *orders,BOOL didReachEnd, NSError *error))completion {
    NSString *endPoint = [NSString stringWithFormat:@"/mall/distOrders/status/%d",(int)MLAMOrderStateCompleted];
